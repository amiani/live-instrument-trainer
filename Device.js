...
exports.Device = function(live, devicePath) {
  ...
  this.getParams = function() {
    ...
    for (var i = 0; i != numParams; i++) {
      var param = new DeviceParameter(joinPaths(paramsPath, i))
  ...
  this.setParams = function(group) {
    for (var i  = 0; i != group.length; i++) {
      var param = this.params[group[i].name]
      this.live.goto(param.path)
      switch (group[i].action) {
        case 'reset':
          this.live.set("value", param.reset_value)
          break
        case 'set':
          this.live.set("value", group[i].value)
          break
        case 'set_random':
          var min = group[i].min
          var max = group[i].max
